...
export function printDoc(doc, type, brand) {
  ...
  const itemsHtml = items.length ? `
    <table style="width:100%;border-collapse:collapse;margin-bottom:32px;font-size:13px;">
      <thead><tr style="background:#1A1410;color:#fff;">
      ...
      <tbody>${items.map((i, idx) => `
        <tr style="background:${idx % 2 === 0 ? '#FAFAF9' : '#fff'};">
          ...
          <td style="padding:14px 16px;text-align:center;color:#7A6E62;">${i.qty ?? 1}</td>
          <td style="padding:14px 16px;text-align:right;color:#7A6E62;">${i.rate || '—'}</td>
...
<style>
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
  html,body{font-family:'Inter',-apple-system,sans-serif;color:#1A1410;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  .page{width:210mm;min-height:297mm;margin:0 auto;padding:72px;position:relative;background:#fff;}
  .wm{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-40deg);font-size:100px;font-weight:900;opacity:.025;white-space:nowrap;pointer-events:none;color:#1A1410;z-index:0;}
  .c{position:relative;z-index:1;}
  @page{size:A4;margin:0;}
  @media print{.page{box-shadow:none!important;}button{display:none!important;}}
  @media screen{.page{box-shadow:0 0 60px rgba(0,0,0,.12);margin:40px auto;border-radius:4px;}body{background:#E8E3DD;}}
</style></head><body>
<div class="page">
  ...
  <div class="c">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-top:40px;margin-bottom:48px;padding-bottom:32px;border-bottom:1.5px solid #E8E3DD;">
      <div style="display:flex;align-items:center;gap:18px;">
        ...
        <div>
          ...
          <div style="font-size:11px;color:#B5AFA9;margin-top:2px;">${brand.address || brand.location || 'Accra, Ghana'}</div>
          <div style="font-size:11px;color:#B5AFA9;">${brand.phone || ''}${brand.email ? ' · ' + brand.email : ''}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:36px;">
      <div style="padding:20px 24px;border:1.5px solid #E8E3DD;border-radius:14px;">
        ...
        <div style="font-size:12px;color:#7A6E62;">${doc.clientEmail || ''}</div>
        <div style="font-size:12px;color:#7A6E62;">${doc.clientPhone || ''}</div>
      </div>
      <div style="padding:20px 24px;border:1.5px solid #E8E3DD;border-radius:14px;">
        ...
        <div style="font-size:12px;color:#7A6E62;">${brand.address || brand.location || 'Accra, Ghana'}</div>
        <div style="font-size:12px;color:#7A6E62;">${brand.phone || ''}</div>
        <div style="font-size:12px;color:#7A6E62;">${brand.email || ''}</div>
      </div>
    </div>
    ...
    <div style="display:flex;justify-content:flex-end;margin-bottom:32px;">
      <div style="min-width:260px;background:#1A1410;border-radius:16px;padding:20px 24px;display:flex;justify-content:space-between;align-items:center;">
    ...
    ${doc.notes ? `<div style="margin-bottom:24px;padding:16px 20px;background:#FFFBF0;border-radius:12px;border:1px solid #F0E5C8;"><div style="font-size:9px;text-transform:uppercase;letter-spacing:.14em;color:${ac};font-weight:800;margin-bottom:8px;">Notes</div><div style="font-size:12px;color:#7A6E62;line-height:1.7;">${doc.notes}</div></div>` : ''}
    ${brand.bankDetails ? `<div style="margin-bottom:24px;padding:16px 20px;background:#F9F7F4;border-radius:12px;border:1px solid #E8E3DD;"><div style="font-size:9px;text-transform:uppercase;letter-spacing:.14em;color:${ac};font-weight:800;margin-bottom:8px;">Payment Details</div><div style="font-size:12px;color:#7A6E62;line-height:1.7;">${brand.bankDetails}</div></div>` : ''}

    <div style="margin-top:40px;padding-top:20px;border-top:1.5px solid #E8E3DD;display:flex;justify-content:space-between;align-items:flex-end;">
      <div style="font-size:11px;color:#B5AFA9;line-height:1.7;">Thank you for your business.<br/>${co} — ${brand.website || 'www.glasstechfab.com'}</div>
      <div style="text-align:right;">
        <div style="font-size:9px;text-transform:uppercase;letter-spacing:.14em;color:#B5AFA9;font-weight:700;margin-bottom:28px;">Authorised Signature</div>
        <div style="width:120px;height:1px;background:#B5AFA9;margin-left:auto;"></div>
