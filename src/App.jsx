...
export default function App() {
  ...
  const migrateToFirebase = async () => {
    ...
    try {
      ...
      const DEMO_ACCOUNTS = [
        ...
        ...(_clientUid1 ? [{ email: 'client@glasstechfab.com', role: 'client', name: 'Elite Client', username: 'elite_finish', uid: _clientUid1 }] : []),
      ];
  ...
  const sendWhatsAppUpdate = async (clientId, projectId, stageName) => {
    ...
    try {
      ...
      const message = stageName.includes(' ') ? stageName : `Hello ${c.name}, your project "${p.project || p.title}" has moved to the ${stageName} phase. - Glasstech Fabrications`;
  ...
  const createClient = async (data) => {
    try {
      ...
      await createNotification(id, 'Welcome to Glasstech! Your project portal is now active.', 'success', '/portal');
  ...
  const createClientProject = async (data) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'projects', docRef.id, 'messages'), {
        ...
        senderRole: 'system', senderId: 'system', senderName: 'Glasstech',
  ...
  const addProjectMessage = async (projectId, text, senderRole = 'admin', isInternal = false) => {
    ...
    const senderName = user?.name || user?.displayName || 'Glasstech Team';
    try {
      await addDoc(collection(db, 'projects', projectId, 'messages'), {
  ...
  const verifyOTP = async (phone, code) => {
    try {
      ...
      if (!userDoc) {
        throw new Error("This number isn't registered. Contact Glasstech to set up your account.");
      }
