  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 24, width: '100%', maxWidth: 480, boxShadow: '0 32px 80px rgba(0,0,0,.2)', display: 'flex', flexDirection: 'column', maxHeight: '80vh' }}>
        {/* Header */}
        <div style={{ padding: '28px 28px 20px', borderBottom: '1px solid #F0EBE5', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#C8A96E', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 4 }}>Client Assignment</div>
              <div style={{ fontSize: 20, fontWeight: 900, color: '#1A1410' }}>{member.name}</div>
            </div>
            <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #F0EBE5', background: '#F9F7F4', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={14} /></button>
          </div>
          <div style={{ position: 'relative' }}>
            <Search size={13} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#B5AFA9' }} />
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search clients..."
              style={{ width: '100%', padding: '9px 12px 9px 34px', borderRadius: 10, border: '1.5px solid #E8E3DD', fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 20px' }}>
          {filtered.length === 0 ? (
            <div style={{ padding: 32, textAlign: 'center', color: '#B5AFA9', fontSize: 13 }}>No clients found</div>
          ) : filtered.map(c => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isSelected ? '#F0F9FF' : '#F9F7F4',
                  border: `1.5px solid ${isSelected ? '#0284C7' : 'transparent'}`,
                  cursor: 'pointer', textAlign: 'left', transition: 'all .15s',
                }}
              >
                <div style={{ width: 36, height: 36, borderRadius: 10, background: isSelected ? '#0284C7' : '#E8E3DD', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: isSelected ? '#fff' : '#B5AFA9', flexShrink: 0 }}>
                  {(c.name || 'C')[0].toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#1A1410' }}>{c.name}</div>
                  <div style={{ fontSize: 11, color: '#B5AFA9' }}>{c.phone || c.email || '—'}</div>
                </div>
        ...
        {/* Footer */}
        <div style={{ padding: '16px 28px 24px', borderTop: '1px solid #F0EBE5', flexShrink: 0, display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, height: 46, borderRadius: 12, border: '1.5px solid #E8E3DD', background: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
          <button
            onClick={handleSave}
            disabled={saving}
            style={{ flex: 2, height: 46, borderRadius: 12, background: saved ? '#16A34A' : '#1A1410', color: '#fff', border: 'none', fontSize: 13, fontWeight: 800, cursor: 'pointer', transition: 'background .2s' }}
          >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p className="lxf" style={{ color: '#B5AFA9', fontSize: 13, marginTop: 4 }}>Manage team accounts, certifications, and client assignments.</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={() => { setShowRepair(true); setRepairResult(null); setRepairForm({ email: '', name: '', role: 'Technician' }); }}
            style={{ padding: '10px 16px', fontSize: 13, gap: 8, display: 'flex', alignItems: 'center', borderRadius: 12, border: '1.5px solid #F0EBE5', background: '#F9F7F4', cursor: 'pointer', fontWeight: 700, color: '#7A6E62', fontFamily: 'inherit' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {[
          { label: 'Total Staff', value: team.length, color: '#1A1410' },
          ...
          <div key={s.label} className="p-card" style={{ padding: 20, textAlign: 'center' }}>
            ...
            <div className="lxf" style={{ fontSize: 11, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '.08em' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="p-card" style={{ overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
              ...
              <tr key={m.id} className="t-row">
                <td style={{ padding: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      ...
                      <div className="lxf" style={{ fontSize: 11, color: '#B5AFA9' }}>{m.email || m.phone || '—'}</div>
                    </div>
                  </div>
                </td>
                <td style={{ padding: '16px' }}>
                  <div className="lxf" style={{ fontSize: 13, color: '#1A1410' }}>{m.role || m.jobRole || '—'}</div>
                </td>
                <td style={{ padding: '16px' }}>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {BADGES.map(b => (
                      <button key={b.id} onClick={() => toggleBadge(m, b.id)} style={{
                        ...
                        border: `1.5px solid ${(m.certs || []).includes(b.id) ? b.color : '#E8E3DD'}`,
                        background: (m.certs || []).includes(b.id) ? `${b.color}15` : 'none',
                        color: (m.certs || []).includes(b.id) ? b.color : '#B5AFA9',
                ...
                <td style={{ padding: '16px' }}>
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, background: '#F9F7F4', border: '1px solid #F0EBE5', fontSize: 12, fontWeight: 700, cursor: 'pointer', color: '#7A6E62' }}
                  >
                    <UserCog size={13} />
                ...
                <td style={{ padding: '16px' }}>
                  <select
                    ...
                    style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #F0EBE5', fontSize: 12, fontWeight: 700, background: '#fff', cursor: 'pointer' }}
                  >
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: resetStates[m.id] === 'sent' ? '#F0FDF4' : '#F9F7F4',
                        border: `1px solid ${resetStates[m.id] === 'sent' ? '#16A34A40' : '#F0EBE5'}`,
                        fontSize: 11, fontWeight: 700, cursor: 'pointer',
                        color: resetStates[m.id] === 'sent' ? '#16A34A' : '#7A6E62',
                      }}
                    >
                      <Mail size={12} />
                    ...
                    {/* Delete */}
                    <button onClick={() => deleteM(m.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B5AFA9', padding: 6 }}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
            ...
            {team.length === 0 && (
              <tr><td colSpan={6} style={{ padding: 48, textAlign: 'center', color: '#B5AFA9' }}>
                <Users size={40} color="#E8E3DD" style={{ marginBottom: 12 }} />
          ...
          <div className="modal-box lxf" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                ...
                <p className="lxf" style={{ color: '#B5AFA9', fontSize: 13 }}>Re-links a Firebase Auth account that's missing from the staff register. Use this for accounts created before the system update.</p>
              </div>
              <button onClick={() => setShowRepair(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B5AFA9', flexShrink: 0 }}><X size={20} /></button>
            </div>
            ...
            {repairResult ? (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                {repairResult.success ? (
                  <>
                    ...
                    <p className="lxf" style={{ color: '#6A635C', fontSize: 13, marginBottom: 20 }}>
              ...
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label className="lxf" style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Email Address *</label>
                  ...
                  <div>
                    <label className="lxf" style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Full Name (if missing)</label>
                  ...
                  <div>
                    <label className="lxf" style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Role</label>
            ...
            {!created ? (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                  <div>
                    ...
                    <p className="lxf" style={{ color: '#B5AFA9', fontSize: 13 }}>Staff can log in and manage their assigned clients.</p>
                  </div>
                  <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B5AFA9' }}><X size={20} /></button>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label className="lxf" style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Full Name *</label>
              ...
              <>
                <div style={{ textAlign: 'center', marginBottom: 24 }}>
                  ...
                  <p className="lxf" style={{ color: '#B5AFA9', fontSize: 13 }}>Share these credentials with <strong>{created.name}</strong>. They must change their password on first login.</p>
                </div>
                <div style={{ background: '#F9F7F4', borderRadius: 14, padding: 20, marginBottom: 20 }}>
                  {[
                    { label: 'Name', value: created.name },
                    { label: 'Email / Login', value: created.email },
                  ].map(row => (
                    <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid #F0EBE5' }}>
                      <span className="lxf" style={{ fontSize: 12, color: '#B5AFA9', fontWeight: 700 }}>{row.label}</span>
                      <span className="lxf" style={{ fontSize: 13, fontWeight: 700, color: '#1A1410' }}>{row.value}</span>
                    </div>
                  ))}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span className="lxf" style={{ fontSize: 12, color: '#B5AFA9', fontWeight: 700 }}>Temp Password</span>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span className="lxf" style={{ fontSize: 13, fontWeight: 800, color: '#1A1410', fontFamily: 'monospace', letterSpacing: 1 }}>
                        {showPw ? created.password : '●'.repeat(created.password.length)}
                      </span>
                      <button onClick={() => setShowPw(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B5AFA9', padding: 0 }}>
