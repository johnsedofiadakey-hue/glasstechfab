    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {slides.map((s, i) => (
            <div key={s.title || i} className="p-card" style={{ padding: 20, background: '#F9F7F4', border: '1px solid rgba(0,0,0,.04)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr auto', gap: 20, alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ height: 100, borderRadius: 6, overflow: 'hidden', background: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {s.img ? <img src={s.img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <ImgIcon size={24} color="#B5AFA9" />}
                  </div>
      ...
      {/* CTA Section */}
      <div style={{ borderTop: '1px solid #F0EBE5', paddingTop: 32 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: '#B5AFA9', marginBottom: 24 }}>
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Live Preview</div>
        <div style={{ aspectRatio: '16/9', borderRadius: 16, overflow: 'hidden', border: '1px solid #F0EBE5', maxWidth: 700 }}>
          <iframe
            key={previewId}
        ...
        <div style={{ marginTop: 8, fontSize: 11, color: '#B5AFA9' }}>Video ID: <code>{previewId}</code></div>
      </div>
    </div>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#B5AFA9', marginTop: 4 }}>The dark strip of numbers between the hero and services sections.</div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {items.map((s, i) => (
          <div key={s._id} style={{ display: 'grid', gridTemplateColumns: '130px 1fr 130px auto', gap: 12, alignItems: 'center', background: '#F9F7F4', padding: '16px 20px', borderRadius: 12 }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#B5AFA9', textTransform: 'uppercase', marginBottom: 6 }}>Number / Value</div>
              <input className="p-inp" placeholder="e.g. 200+" value={s.value} onChange={e => update(i, 'value', e.target.value)} />
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#B5AFA9', textTransform: 'uppercase', marginBottom: 6 }}>Desktop Label</div>
              <input className="p-inp" placeholder="e.g. Projects Delivered" value={s.label} onChange={e => update(i, 'label', e.target.value)} />
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#B5AFA9', textTransform: 'uppercase', marginBottom: 6 }}>Mobile Label</div>
      ...
      {/* Live preview */}
      <div style={{ background: '#1A1410', borderRadius: 16, padding: '28px 32px', display: 'grid', gridTemplateColumns: `repeat(${items.length}, 1fr)`, gap: 16 }}>
        {items.map((s, i) => (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#B5AFA9', marginTop: 4 }}>Reason cards displayed in the "Built to a higher standard" section.</div>
        </div>
          ...
          <div key={r._id} className="p-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ fontSize: 12, color: '#7A6E62', fontWeight: 600 }}>Pick icon:</div>
              </div>
              <button onClick={() => setItems(items.filter((_, idx) => idx !== i))} style={{ color: '#ff4444', border: 'none', background: 'none', cursor: 'pointer', padding: 4 }}><Trash2 size={16} /></button>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {EMOJI_OPTIONS.map(em => (
                <button key={em} onClick={() => update(i, 'emoji', em)}
                  style={{ fontSize: 18, padding: '4px 6px', borderRadius: 8, cursor: 'pointer', border: r.emoji === em ? `2px solid ${ac}` : '1px solid #E8E3DD', background: r.emoji === em ? `${ac}12` : '#fff', lineHeight: 1 }}
                >{em}</button>
              ))}
            </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#B5AFA9', marginTop: 4 }}>The 3-card preview grid under "Structural Precision." Up to 6 cards shown.</div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {items.map((s, i) => (
          <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '56px 1fr 1fr auto', gap: 16, alignItems: 'center', background: '#F9F7F4', padding: '16px 20px', borderRadius: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#B5AFA9', textTransform: 'uppercase', marginBottom: 6 }}>Card Title</div>
              <input className="p-inp" placeholder="Service name" value={s.name} onChange={e => update(i, 'name', e.target.value)} />
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#B5AFA9', textTransform: 'uppercase', marginBottom: 6 }}>Short Description</div>
...
export default function AdminCMS({ content, syncCMS, brand, onPreview, notify, ...props }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, background: '#F9F7F4', padding: 6, borderRadius: 14 }}>
        {tabs.map(t => (
