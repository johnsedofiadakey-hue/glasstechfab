      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <select
            ...
            style={{ height: 38, borderRadius: 10, border: '1px solid #F0EBE5', padding: '0 12px', fontSize: 12, fontWeight: 700, background: '#fff', color: '#1A1410', cursor: 'pointer' }}
          >
            {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <div style={{ display: 'flex', borderRadius: 10, border: '1px solid #F0EBE5', overflow: 'hidden' }}>
            {['All', 'Q1', 'Q2', 'Q3', 'Q4'].map(q => (
              <button
                key={q}
                onClick={() => setSelectedQuarter(q)}
                style={{ padding: '8px 14px', border: 'none', borderRight: q !== 'Q4' ? '1px solid #F0EBE5' : 'none', background: selectedQuarter === q ? '#1A1410' : '#fff', color: selectedQuarter === q ? '#fff' : '#7A6E62', fontSize: 11, fontWeight: 800, cursor: 'pointer' }}
              >{q}</button>
            ))}
          </div>
          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis stroke="#B5AFA9" fontSize={11} tickFormatter={v => `GH₵${(v/1000).toFixed(0)}k`} />
