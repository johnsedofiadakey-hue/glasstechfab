  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 24 : 32 }}>
      ...
      {clients && clients.length > 0 && (() => {
        ...
        return (
          <div style={{ background: '#fff', border: '1px solid #F0EBE5', borderRadius: 20, padding: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <div>
                ...
                <div className="lxf" style={{ fontSize: 12, color: '#B5AFA9' }}>Projects stuck in their current stage for 5+ days</div>
              </div>
            </div>
            {stuckProjects.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '32px 16px', border: '1px dashed #F0EBE5', borderRadius: 16, color: '#16A34A', fontSize: 13, background: 'rgba(22,163,74,0.03)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {stuckProjects.map(project => {
                  ...
                  return (
                    <div key={project.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', background: '#F9F7F4', borderRadius: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#1A1410', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{project.title || project.name || 'Untitled Project'}</div>
                        <div style={{ fontSize: 11, color: '#7A6E62', marginTop: 2 }}>{project.clientName || ''} · {stageObj?.name || `Stage ${project.stageId}`}</div>
                      </div>
                      ...
                      <button
                        ...
                        style={{ padding: '8px 14px', borderRadius: 10, background: '#1A1410', color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}
                      >
                        View →
                      </button>
                    </div>
            ...
            <div style={{ height: 320, width: '100%', minHeight: 320, background: 'var(--bg-alt)', borderRadius: 16, overflow: 'hidden', position: 'relative' }}>
             <ResponsiveContainer width="100%" height={320} minHeight={320}>
               <AreaChart data={dynamicRevData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                ...
                <YAxis tick={{ fill: '#B5AFA9', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `GH₵${v}k`} />
                <Tooltip 
