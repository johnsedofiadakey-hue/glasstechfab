...
export default function AdminLogistics({ containers = [], workOrders = [], clients = [], brand, ...props }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div className="p-card" style={{ padding: 24, textAlign: 'center', border: `1px dashed ${ac}`, borderRadius: 24 }}>
               ...
               {warehouseCount > 0 ? (
                 <p style={{ fontSize: 12, color: '#B5AFA9', lineHeight: 1.6 }}>
                   <strong style={{ color: '#1A1410' }}>{warehouseCount}</strong> shipment{warehouseCount !== 1 ? 's' : ''} ({warehouseItems} items) at central warehouse awaiting site deployment.
                 </p>
               ) : (
                 <p style={{ fontSize: 12, color: '#B5AFA9', lineHeight: 1.6 }}>No shipments currently at the warehouse.</p>
               )}
            </div>
         </div>
      </div>
      ...
      {/* NEW CONTAINER MODAL */}
      {showAddContainer && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(26,20,16,0.6)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={() => setShowAddContainer(false)}>
          <div style={{ background: '#fff', borderRadius: 28, padding: 40, width: '100%', maxWidth: 560, boxShadow: '0 40px 80px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              <div>
                ...
                <p className="lxf" style={{ fontSize: 13, color: '#B5AFA9', marginTop: 4 }}>Track a new container or sea freight</p>
              </div>
              <button onClick={() => setShowAddContainer(false)} style={{ background: '#F9F7F4', border: 'none', width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                <X size={18} />
              </button>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
              <button onClick={() => setShowAddContainer(false)} style={{ flex: 1, padding: 16, borderRadius: 14, background: '#F9F7F4', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleCreateContainer} disabled={saving} style={{ flex: 2, padding: 16, borderRadius: 14, background: '#1A1410', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
