       ...
       {tab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} className="fade-in">
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                ...
                <PulseTargetCard label="OPEN TENDERS" value={stats.quotes} target={finSettings.kpiTargets?.quotes ?? 20} icon={<FileText size={20}/>} color="#1A1410" trend={12} sub={`Value: ${formatMoney((props.proposals||[]).filter(p=>p.status==='Pending').reduce((a,b)=>a+(parseAmount(b.amount)||0),0),'GHS')}`} />
                  ...
                  <tbody>
                     ...
                     {(tab === 'sales' ? invoices : (props.proposals || [])).map(item => (
                       <tr key={item.id} style={{ borderBottom: '1px solid var(--border)' }}>
                          <td style={{ padding: '20px 24px', fontSize: 11, fontWeight: 800, fontFamily: 'monospace', color: '#7A6E62' }}>{(item.id || '').slice(0, 12).toUpperCase()}</td>
                          <td style={{ padding: '20px 24px' }}>
                             <div style={{ fontSize: 14, fontWeight: 700 }}>{item.client || item.clientName}</div>
                             <div style={{ fontSize: 11, color: '#B5AFA9' }}>{item.clientEmail || item.title || '—'}</div>
                          </td>
           ...
           <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
             ...
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
               {[
                 ...
                 { label: 'Gross Profit',    value: fmt(totalProfit),   color: totalProfit >= 0 ? '#16A34A' : '#DC2626', bg: '#F9F7F4' },
                 { label: 'Avg Net Margin',  value: `${avgMargin.toFixed(1)}%`, color: avgMargin >= 20 ? '#16A34A' : avgMargin >= 10 ? '#D97706' : '#DC2626', bg: '#F9F7F4' },
               ].map(({ label, value, color, bg }) => (
                 <div key={label} className="p-card" style={{ padding: '20px 24px', background: bg }}>
                   <div style={{ fontSize: 10, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 6 }}>{label}</div>
             ...
             <div className="p-card" style={{ overflow: 'hidden' }}>
               <div style={{ padding: '20px 24px', borderBottom: '1px solid #F0EBE5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                 <div>
                   <div style={{ fontSize: 16, fontWeight: 900, color: '#1A1410' }}>Project P&L Breakdown</div>
                   <div style={{ fontSize: 12, color: '#B5AFA9', marginTop: 2 }}>Cost data entered via the Economics panel in each project.</div>
                 </div>
                 <button
                   onClick={exportCSV}
                   style={{ height: 38, padding: '0 16px', display: 'flex', alignItems: 'center', gap: 6, background: '#fff', border: '1px solid #F0EBE5', borderRadius: 10, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}
                 >
               ...
               {rows.length === 0 ? (
                 <div style={{ padding: 56, textAlign: 'center' }}>
                   <TrendingUp size={40} color="#E8E3DD" style={{ marginBottom: 12 }} />
                   <div style={{ fontSize: 14, fontWeight: 700, color: '#B5AFA9', marginBottom: 4 }}>No cost data yet</div>
                     ...
                     <thead>
                       <tr style={{ background: '#F9F7F4' }}>
                         {['Project', 'Sale Price', 'Product Cost', 'Shipping', 'Installation', 'COGS', 'Gross Profit', 'Margin'].map(h => (
                           <th key={h} style={{ padding: '14px 20px', textAlign: 'left', fontSize: 10, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '.08em', whiteSpace: 'nowrap' }}>{h}</th>
                         ))}
                       </tr>
                     </thead>
                     <tbody>
                       {rows.map(r => {
                         const mc = r.margin !== null ? (r.margin >= 20 ? '#16A34A' : r.margin >= 10 ? '#D97706' : '#DC2626') : '#B5AFA9';
                         return (
                           <tr key={r.id} style={{ borderBottom: '1px solid #F9F7F4' }}>
                             <td style={{ padding: '14px 20px' }}>
                               <div style={{ fontSize: 13, fontWeight: 700, color: '#1A1410' }}>{r.title || r.name || 'Untitled'}</div>
                               <div style={{ fontSize: 11, color: '#B5AFA9', marginTop: 2 }}>{r.id?.slice(0, 8).toUpperCase()}</div>
                             </td>
                             <td style={{ padding: '14px 20px', fontSize: 13, fontWeight: 800, color: '#1A1410', whiteSpace: 'nowrap' }}>{fmt(r.salePrice)}</td>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                     {[
                       ...
                       <button key={t.id} onClick={() => setDraft({ ...draft, invoiceType: t.id })}
                         style={{ flex: 1, minWidth: 120, padding: '12px 16px', borderRadius: 12, border: draft.invoiceType === t.id ? `2px solid ${ac}` : '1px solid #F0EBE5', background: draft.invoiceType === t.id ? `${ac}10` : '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700 }}>
                         {t.icon} {t.label}
                       </button>
                     ))}
                  </div>
