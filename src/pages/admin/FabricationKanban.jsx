          ...
          <div key={s.id} className="kanban-col" style={{ minWidth: 320, flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '4px' }}>
               {jobs.filter(j => j.stage === s.id).map(j => (
                 <div key={j.id} className="p-card kanban-card fade-in" style={{ padding: 20, background: '#fff', borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: j.priority === 'High' ? '1px solid #FEE2E2' : '1px solid transparent' }}>
