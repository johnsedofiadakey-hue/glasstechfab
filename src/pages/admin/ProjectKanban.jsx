...
const KANBAN_COLS = [
  ...
  { id: 'handover',     label: 'Handover',     stages: [7], color: '#C8A96E', bg: 'rgba(200,169,110,0.07)' },
];
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, paddingRight: 64 }}>
          ...
          <div style={{ fontSize: 11, color: '#B5AFA9', display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#7A6E62' }}>{project.budget || '—'}</div>
        {daysInStage !== null && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#B5AFA9', fontWeight: 700 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 9, color: '#B5AFA9', marginTop: 4, textAlign: 'right', fontWeight: 700 }}>{stage.pct}%</div>
      </div>
    </div>
  );
}
    ...
    >
      <div style={{ padding: '16px 16px 14px', position: 'sticky', top: 0, background: 'inherit', borderRadius: '20px 20px 0 0', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: '#B5AFA9', background: '#F0EBE5', padding: '2px 8px', borderRadius: 20 }}>{projects.length}</span>
        </div>
        <div style={{ marginTop: 6, fontSize: 10, color: '#B5AFA9' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end' }} onClick={onClose}>
      <div style={{ width: '100%', maxWidth: 500, height: '100vh', background: '#fff', boxShadow: '-20px 0 60px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        {/* Header */}
        <div style={{ padding: '28px 28px 0', background: '#1A1410', color: '#fff', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            ...
            {confirmDelete && (
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(26,20,16,0.97)', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, borderRadius: 0 }}>
        ...
        <div style={{ flex: 1, padding: 28, overflowY: 'auto' }}>
          ...
          {tab === 'overview' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ padding: 20, background: `${stage.color}10`, border: `1px solid ${stage.color}30`, borderRadius: 16 }}>
                ...
                <p style={{ fontSize: 13, color: '#1A1410', margin: 0, lineHeight: 1.6, fontWeight: 500 }}>{stage.adminPrompt}</p>
                {stageHistoryMap[stageId]?.timestamp && (
                  <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#B5AFA9' }}>
                    <Calendar size={12} /> Entered {fmtDate(stageHistoryMap[stageId].timestamp)}
                    {daysSince(stageHistoryMap[stageId].timestamp) !== null && (
                      <span style={{ fontWeight: 700, color: '#7A6E62' }}>· {daysSince(stageHistoryMap[stageId].timestamp)}d ago</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ padding: 16, background: '#F9F7F4', borderRadius: 14 }}>
                  <div style={{ fontSize: 10, color: '#B5AFA9', textTransform: 'uppercase', fontWeight: 800, marginBottom: 8 }}>Stages Done</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: '#1A1410' }}>{currentIdx}<span style={{ fontSize: 13, color: '#B5AFA9', fontWeight: 500 }}>/{availableStages.length}</span></div>
                </div>
                <div style={{ padding: 16, background: '#F9F7F4', borderRadius: 14 }}>
                  <div style={{ fontSize: 10, color: '#B5AFA9', textTransform: 'uppercase', fontWeight: 800, marginBottom: 8 }}>Invoiced</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: '#1A1410' }}>{projInvoices.filter(i => i.status === 'Paid').length}<span style={{ fontSize: 13, color: '#B5AFA9', fontWeight: 500 }}>/{projInvoices.length}</span></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {nextStage && (
                    <button onClick={advance} style={{ padding: '12px 16px', background: ac, color: '#1A1410', border: 'none', borderRadius: 12, fontSize: 12, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, gridColumn: '1 / -1' }}>
                      <ChevronRight size={14} /> Advance to {nextStage.name}
                    </button>
                  )}
                  <button onClick={() => sendWhatsAppUpdate && sendWhatsAppUpdate(project.clientId, project.id, stage.name)} style={{ padding: '12px 16px', background: '#F9F7F4', color: '#1A1410', border: '1px solid #F0EBE5', borderRadius: 12, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    <MessageCircle size={14} /> Notify Client
                  </button>
                  <button onClick={() => createInvoice && createInvoice({ parentId: project.id, clientId: project.clientId, title: `${stage.name} Payment`, amount: '', status: 'Pending' })} style={{ padding: '12px 16px', background: '#F9F7F4', color: '#1A1410', border: '1px solid #F0EBE5', borderRadius: 12, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          {tab === 'timeline' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 16 }}>Project Journey</div>
                ...
                return (
                  <div key={s.id} style={{ display: 'flex', gap: 12, position: 'relative', opacity: isFuture ? 0.4 : 1 }}>
                    ...
                    {i < availableStages.length - 1 && (
                      <div style={{ position: 'absolute', left: 16, top: 36, width: 2, height: 'calc(100% - 4px)', background: isPast ? s.color : '#F0EBE5', borderRadius: 1 }} />
                    )}
                    {/* Circle */}
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: isPast ? s.color : isCurrent ? '#fff' : '#F0EBE5', border: isCurrent ? `2px solid ${s.color}` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, zIndex: 1 }}>
                      {isPast ? <Check size={14} color="#fff" strokeWidth={3} /> : <span style={{ fontSize: 14 }}>{s.emoji}</span>}
                    </div>
                    {/* Content */}
                    <div style={{ flex: 1, paddingBottom: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                        <span style={{ fontSize: 13, fontWeight: isCurrent ? 700 : 500, color: isCurrent ? '#1A1410' : isPast ? '#7A6E62' : '#B5AFA9' }}>{s.name}</span>
                      ...
                      {enteredDate && (
                        <div style={{ fontSize: 11, color: '#B5AFA9', display: 'flex', gap: 10 }}>
          ...
          {tab === 'finance' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {projInvoices.length === 0 ? (
                <div style={{ padding: 40, textAlign: 'center', color: '#B5AFA9', fontSize: 13, background: '#F9F7F4', borderRadius: 16 }}>No invoices yet for this project.</div>
              ) : (
          ...
          {tab === 'team' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 4 }}>
                Assigned Team — only assigned members can message this client
              </div>
              {teamMembers.length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center', color: '#B5AFA9', fontSize: 13, background: '#F9F7F4', borderRadius: 16 }}>
                ...
                teamMembers.filter(m => m.role !== 'client').map(m => {
                  ...
                  return (
                    <div key={m.id || m.uid} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: isAssigned ? '#F0FDF4' : '#F9F7F4', border: `1px solid ${isAssigned ? '#BBF7D0' : '#F0EBE5'}`, borderRadius: 14, transition: 'all 0.2s' }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: isAssigned ? '#16A34A' : '#E8E3DD', color: isAssigned ? '#fff' : '#7A6E62', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>
                        {(m.name || 'S')[0].toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#1A1410' }}>{m.name}</div>
                        <div style={{ fontSize: 11, color: '#7A6E62' }}>{m.jobRole || m.role}</div>
                      </div>
                      {isAssigned && (
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        style={{ padding: '7px 14px', borderRadius: 10, border: 'none', background: isAssigned ? '#EF4444' : ac, color: isAssigned ? '#fff' : '#1A1410', fontSize: 11, fontWeight: 800, cursor: 'pointer', opacity: isLoading ? 0.6 : 1, flexShrink: 0 }}
                      >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p style={{ color: '#B5AFA9', fontSize: 13, margin: '4px 0 0' }}>Drag projects between columns — click to open detail and advance stages</p>
        </div>
