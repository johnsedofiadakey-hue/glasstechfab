        ...
        <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr 1fr' : 'repeat(3, 1fr)', gap: mob ? 12 : 20 }}>
          {items.slice(0, 9).map((s, i) => (
            ...
            >
              ...
              {!mob && <p style={{ color: 'rgba(26,20,16,0.55)', lineHeight: 1.7, marginBottom: 20, fontSize: 13 }}>{s.short || s.desc}</p>}
...
function StatsBar({ brand, stats }) {
  ...
  return (
    <section style={{ background: '#1A1410', padding: '48px 5vw' }}>
...
function StatsBarMobile({ brand, stats }) {
  ...
  const items = (stats && stats.length) ? stats : DEFAULT_STATS;
  return (
    <section style={{ background: '#1A1410', padding: '40px 24px' }}>
...
function WhyGlasstech({ brand, navigate, reasons }) {
  const ac = brand.color || AC;
  const winW = useWindowWidth();
  const mob = isMob(winW);
    ...
    <section style={{ padding: mob ? '80px 24px' : '140px 5vw', background: '#fff' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: mob ? 48 : 80 }}>
          <span style={{ color: ac, fontSize: 10, fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase' }}>WHY GLASSTECH</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: mob ? 20 : 32 }}>
          {items.map((r, i) => (
            <div key={r.title || i} style={{
              ...
              background: '#F9F7F4', borderRadius: 24,
            ...
            >
              ...
              <div>
                ...
                <p style={{ fontSize: 14, color: 'rgba(26,20,16,0.6)', lineHeight: 1.7, margin: 0 }}>{r.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: mob ? 40 : 64, textAlign: 'center' }}>
          <button
            ...
            style={{ padding: '18px 48px', background: '#1A1410', color: '#fff', border: 'none', borderRadius: 14, fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10 }}
...
const DEFAULT_TESTIMONIALS = [
  { author: 'Kwame Asante',      text: 'Glasstech transformed our office with exceptional precision. The structural glazing exceeded every expectation — on time and on budget.',  projectTitle: 'Airport Hills Commercial Tower',  rating: 5 },
...
function TestimonialsSection({ brand, testimonials: propTestimonials, cmsTestimonials }) {
  ...
  return (
    <section style={{ padding: mob ? '80px 24px' : '140px 5vw', background: '#F9F7F4' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 28, padding: mob ? '40px 28px' : '64px 80px', boxShadow: '0 24px 60px rgba(0,0,0,0.06)', position: 'relative', minHeight: 240 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: 'rgba(26,20,16,0.45)', marginTop: 4 }}>{item.projectTitle || item.role}</div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 28 }}>
            {items.map((_, i) => (
              <button
                key={i}
                onClick={() => setActive(i)}
                style={{ width: i === active ? 28 : 8, height: 8, borderRadius: 4, background: i === active ? ac : '#E8E3DD', border: 'none', cursor: 'pointer', transition: 'all 0.3s', padding: 0 }}
              />
            ))}
          </div>
        )}
...
export default function PublicSite({ brand, setPage, page, onPortal, user, content, navigate, submitContact, testimonials = [] }) {
  ...
  const render = () => {
    ...
    if (p === 'home') return (
      <>
        ...
        <WhyGlasstech brand={brand} navigate={navigate} reasons={content?.whyUs} />
        <TestimonialsSection brand={brand} testimonials={testimonials} cmsTestimonials={content?.testimonials} />
        <section style={{ padding: mob ? '80px 24px' : '120px 5vw', background: '#1A1410', color: '#fff', textAlign: 'center' }}>
          <div style={{ maxWidth: 700, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={() => navigate('/?page=contact')} style={{ padding: '18px 48px', background: brand.color || AC, color: '#1A1410', border: 'none', borderRadius: 14, fontWeight: 900, fontSize: 15, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
  ...
  return (
    <div style={{ background: LIGHT_BG }}>
      ...
      <a 
        href={`https://wa.me/${brand.whatsapp || '233598455012'}?text=${encodeURIComponent("Hi, I'm interested in a glass installation quote from Glasstech Fabrications.")}`}
        target="_blank" 
