    ...
    <div style={{ background: '#fff', borderRadius: 20, padding: 24, boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#1A1410', marginBottom: 4, lineHeight: 1.3 }}>
            {project.title || project.name || 'Untitled Project'}
          </div>
          <div style={{ fontSize: 12, color: '#7A6E62' }}>{project.clientName || 'Client'}</div>
        </div>
        <div style={{
      ...
      {/* Stage badge */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#F9F7F4', borderRadius: 10, marginBottom: 20 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 11, fontWeight: 800, color: '#7A6E62', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 8 }}>
          Add a note
        </label>
        <textarea
          value={note}
          ...
          style={{
            ...
            fontSize: 13, fontFamily: 'inherit', resize: 'vertical', color: '#1A1410',
            background: '#F9F7F4', boxSizing: 'border-box', outline: 'none'
          }}
        />
        <button
          ...
          style={{
            ...
            background: note.trim() ? '#1A1410' : '#E5E0D8',
      ...
      <div>
        ...
        <label style={{
          ...
          background: photoSent ? '#F0FDF4' : '#F9F7F4',
          color: photoSent ? '#16A34A' : '#1A1410',
    ...
    <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
      <button
        ...
        style={{
          ...
          background: 'transparent', border: 'none', cursor: 'pointer', borderBottom: open ? '1px solid #F0EBE5' : 'none'
        }}
      >
        <span style={{ fontSize: 15, fontWeight: 700, color: '#1A1410' }}>All Assigned Projects ({projects.length})</span>
        {open ? <ChevronUp size={18} color="#7A6E62" /> : <ChevronDown size={18} color="#7A6E62" />}
      </button>
      {open && (
        <div style={{ padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {projects.map(p => (
            <div key={p.id} style={{ padding: '12px 16px', background: '#F9F7F4', borderRadius: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1A1410' }}>{p.title || p.name || 'Untitled'}</div>
              <div style={{ fontSize: 11, color: '#7A6E62', marginTop: 4 }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#7A6E62' }}>
              Today's Jobs
            </div>
            {todayProjects.length > 0 && (
              <div style={{ background: '#1A1410', color: '#fff', padding: '2px 8px', borderRadius: 100, fontSize: 10, fontWeight: 800 }}>
          ...
          {todayProjects.length === 0 ? (
            <div style={{
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: '#1A1410', marginBottom: 8 }}>
                No jobs assigned for today.
              </div>
              <div style={{ fontSize: 13, color: '#7A6E62' }}>Contact your supervisor for your assignment.</div>
            </div>
          ) : (
        ...
        {allAssigned.length > 0 && (
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#7A6E62', marginBottom: 16 }}>
              All Assignments
            </div>
        ...
        {/* Footer */}
        <div style={{ textAlign: 'center', padding: '16px 0', fontSize: 11, color: '#B5AFA9' }}>
          {brand?.name || 'GlassTechFab'} Field App
        </div>
      </div>
    </div>
  );
