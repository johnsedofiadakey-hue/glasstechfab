...
const ProductCard = ({ product, onClick, ac, mob, onCompare, isComparing, waNumber }) => {
  ...
  const waMsg = encodeURIComponent(`Hi Glasstech, I'm interested in: ${product.name}. Please send me a quote.`);
  ...
  return (
    ...
    >
      <div style={{ height: mob ? 220 : 260, background: '#F9F7F4', position: 'relative', overflow: 'hidden' }}>
        <img
          src={product.img}
          alt={product.name}
      ...
      <div style={{ padding: mob ? 16 : 20, flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <p style={{ fontSize: mob ? 11 : 12, color: 'rgba(26,20,16,0.5)', lineHeight: 1.5, margin: '0 0 16px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8, borderTop: '1px solid #f5f5f5', paddingTop: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: DARK_TEXT, fontSize: 11, fontWeight: 800, textTransform: 'uppercase', padding: '10px 12px', background: '#F9F7F4', borderRadius: 10 }}>
    ...
    <div style={{ minHeight: '100vh', background: LIGHT_BG, color: DARK_TEXT, fontFamily: 'var(--font-p)', position: 'relative' }}>
      ...
      <main style={{ padding: mob ? '100px 20px 100px' : '160px 5vw 100px', maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: mob ? 32 : 48 }}>
          ...
          <p style={{ color: 'rgba(26,20,16,0.5)', fontSize: mob ? 14 : 17, maxWidth: 700, lineHeight: 1.7 }}>
            Glass, tiles, bathrooms, kitchens, wardrobes, doors, electrical and plumbing materials — all sourced directly and available for supply or full installation across Ghana.
          </p>
        </div>

        {/* Top CTA strip */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, padding: '16px 24px', background: '#1A1410', borderRadius: 16, marginBottom: 40 }}>
          <div>
          ...
          <a
            href={`https://wa.me/${waNumber}?text=${encodeURIComponent('Hi Glasstech, I need a custom material sourced. Can you help?')}`}
            target="_blank" rel="noopener noreferrer"
        ...
        <div style={{ 
          ...
          {(filtered || []).length === 0 && (
            <div style={{ gridColumn: '1 / -1', padding: '80px 0', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, color: '#B5AFA9', marginBottom: 24 }}>We can source any material from China — contact us for a custom order.</div>
              <a
                href={`https://wa.me/${waNumber}?text=${encodeURIComponent('Hi Glasstech, I\'m looking for a product you may not have listed. Can you help source it?')}`}
                target="_blank" rel="noopener noreferrer"
