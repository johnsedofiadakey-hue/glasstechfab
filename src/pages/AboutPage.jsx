...
export default function AboutPage({ brand, content, navigate }) {
  ...
  return (
    <div style={{ paddingTop: mob ? 80 : 120, fontFamily: 'Inter, Satoshi, sans-serif' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ padding: mob ? '60px 24px 40px' : '80px 5vw 60px', background: '#F9F7F4' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          <span style={{ color: ac, fontSize: 10, fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase' }}>
            About Glasstech Fabrications
          </span>
          ...
          <p style={{ fontSize: mob ? 16 : 22, color: 'rgba(26,20,16,0.55)', maxWidth: 760, lineHeight: 1.7 }}>
      ...
      <section style={{ padding: mob ? '60px 24px' : '100px 5vw', background: '#fff' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'rgba(26,20,16,0.6)', lineHeight: 1.85, fontSize: 16, marginBottom: 32 }}>
              {data.bio || 'John Dakey leads Glasstech with a commitment to sub-millimeter precision and aesthetic excellence. From Spintex to the most exclusive developments in Accra, his vision is to provide a "million-dollar finish" for every project, leveraging global sourcing and local technical expertise.'}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: 24, background: '#F9F7F4', borderRadius: 20, border: '1px solid #F0EBE5' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(26,20,16,0.5)' }}>{brand.location || 'Spintex Road Industrial Area, Accra, Ghana'}</div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: 24, background: '#F9F7F4', borderRadius: 20, border: '1px solid #F0EBE5' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(26,20,16,0.5)' }}>All 16 regions of Ghana — field teams deployed nationwide</div>
                </div>
              </div>
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: '#B5AFA9' }}>Delivered since 2012</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── VALUES ── */}
      <section style={{ padding: mob ? '60px 24px' : '100px 5vw', background: '#F9F7F4' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
            ...
            {VALUES.map((v, i) => (
              <div
                ...
                style={{
                  ...
                  border: '1px solid',
                  borderColor: i === 0 ? DARK_TEXT : '#F0EBE5',
      ...
      <section style={{ padding: mob ? '60px 24px' : '100px 5vw', background: '#fff' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: mob ? 20 : '50%', top: 0, bottom: 0, width: 2, background: '#F0EBE5', transform: mob ? 'none' : 'translateX(-50%)' }} />
              ...
              {TIMELINE.map((item, i) => (
                ...
                >
                  ...
                  <div style={{
                    ...
                    background: '#F9F7F4', borderRadius: 20,
          ...
          <div style={{ maxWidth: 1400, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 20 }}>
              {visibleTeam.slice(0, 8).map(m => (
                <div key={m.id || m.uid} style={{ background: '#fff', borderRadius: 24, overflow: 'hidden', border: '1px solid #F0EBE5' }}>
      ...
      {/* ── CERTIFICATIONS ── */}
      <section style={{ padding: mob ? '60px 24px' : '80px 5vw', background: '#fff', borderTop: '1px solid #F0EBE5' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 16 }}>
            {[
              ...
              <div key={c.label} style={{ padding: 28, background: '#F9F7F4', borderRadius: 20, border: '1px solid #F0EBE5', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ fontSize: 12, color: '#B5AFA9', fontWeight: 600 }}>{c.sub}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            {navigate && (
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '18px 40px', background: ac, color: '#1A1410', border: 'none', borderRadius: 14, fontWeight: 900, fontSize: 15, cursor: 'pointer' }}
              >
                Get a Free Quote <ArrowRight size={16} />
              </button>
            )}
            <a
              href={`https://wa.me/${brand.whatsapp || '233598455012'}?text=${encodeURIComponent("Hi Glasstech, I saw your About page and I'd like to discuss a project.")}`}
