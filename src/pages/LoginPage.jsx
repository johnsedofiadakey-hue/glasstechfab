    ...
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      <button
        ...
        style={{
          ...
          border: '1.5px solid #E8E3DD', background: '#F9F7F4',
      ...
      >
        ...
        <span style={{ fontWeight: 700, color: '#1A1410' }}>{value.code}</span>
        <span style={{ fontSize: 10, color: '#B5AFA9' }}>▾</span>
      </button>
      ...
      {open && !isMob && (
        <div style={{
          ...
          background: '#fff', borderRadius: 16, border: '1.5px solid #E8E3DD',
          boxShadow: '0 16px 40px rgba(26,20,16,.12)', width: 280, overflow: 'hidden',
        }}>
          <div style={{ padding: '10px 12px', borderBottom: '1px solid #F0EBE5', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Search size={14} color="#B5AFA9" />
            <input ref={searchRef} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search country or code..."
              style={{ flex: 1, border: 'none', outline: 'none', fontSize: 14, color: '#1A1410', fontFamily: 'inherit', background: 'transparent' }} />
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}><X size={13} color="#B5AFA9" /></button>}
          </div>
          <div style={{ maxHeight: 280, overflowY: 'auto' }}>
            {filtered.length === 0
              ? <div style={{ padding: '20px 16px', textAlign: 'center', color: '#B5AFA9', fontSize: 13 }}>No results</div>
              : filtered.map(c => (
                <button key={c.code + c.name} type="button" onClick={() => { onChange(c); close(); }}
                  style={{ width: '100%', padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 10, background: c.code === value.code && c.name === value.name ? '#F9F7F4' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
                  <span style={{ fontSize: 18, width: 24, textAlign: 'center' }}>{c.flag}</span>
                  <span style={{ flex: 1, fontSize: 13, color: '#1A1410', fontWeight: 500 }}>{c.name}</span>
                  <span style={{ fontSize: 12, color: '#B5AFA9', fontWeight: 700 }}>{c.code}</span>
                </button>
              ))
            }
          </div>
      ...
      {open && isMob && (
        <>
          <div onClick={close} style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(26,20,16,.55)', backdropFilter: 'blur(4px)' }} />
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center', padding: '12px 0 4px' }}>
              <div style={{ width: 36, height: 4, borderRadius: 2, background: '#E8E3DD' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px 12px' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: '#1A1410' }}>Select Country</div>
              <button onClick={close} style={{ width: 36, height: 36, borderRadius: 10, background: '#F0EBE5', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', touchAction: 'manipulation' }}>
                <X size={16} color="#7A6E62" />
              </button>
            </div>
            <div style={{ padding: '0 16px 12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F9F7F4', borderRadius: 14, padding: '12px 16px', border: '1.5px solid #E8E3DD' }}>
                <Search size={16} color="#B5AFA9" />
                <input ref={searchRef} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search country or code..."
                  style={{ flex: 1, border: 'none', outline: 'none', fontSize: 16, color: '#1A1410', fontFamily: 'inherit', background: 'transparent' }} />
                {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', touchAction: 'manipulation' }}><X size={14} color="#B5AFA9" /></button>}
              </div>
            </div>
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {filtered.length === 0
                ? <div style={{ padding: '32px 16px', textAlign: 'center', color: '#B5AFA9', fontSize: 14 }}>No results</div>
                : filtered.map(c => (
                  <button key={c.code + c.name} type="button" onClick={() => { onChange(c); close(); }}
                    style={{ width: '100%', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14, background: c.code === value.code && c.name === value.name ? '#F9F7F4' : 'none', border: 'none', borderBottom: '1px solid #F9F7F4', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', touchAction: 'manipulation' }}>
                    <span style={{ fontSize: 22, width: 28, textAlign: 'center' }}>{c.flag}</span>
                    <span style={{ flex: 1, fontSize: 15, color: '#1A1410', fontWeight: 600 }}>{c.name}</span>
                    <span style={{ fontSize: 13, color: '#B5AFA9', fontWeight: 700 }}>{c.code}</span>
                  </button>
                ))
              }
            </div>
      ...
      {digits.map((d, i) => (
        <input
          ...
          style={{
            ...
            fontWeight: 900,
            borderRadius: 16,
            border: `2px solid ${d ? '#1A1410' : '#E8E3DD'}`,
            background: d ? '#F9F7F4' : '#fff',
            color: '#1A1410',
            outline: 'none',
            fontFamily: 'inherit',
    ...
    <div style={{
      ...
      {!isMobile && (
        <div style={{ width: '100%', maxWidth: 440, marginBottom: 20 }}>
          <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: isAdmin ? '#625C54' : '#B5AFA9', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500, padding: 0 }}>
      ...
      <div style={{
        ...
        background: isAdmin ? '#1A1410' : '#fff',
        border: isMobile ? 'none' : `1px solid ${isAdmin ? 'rgba(255,255,255,.06)' : '#F0EBE5'}`,
        ...
        {isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, position: 'relative', zIndex: 1 }}>
            <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 15, color: isAdmin ? '#625C54' : '#B5AFA9', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: '8px 0', touchAction: 'manipulation', minHeight: 44 }}>
              <ChevronLeft size={18} /> Back
            </button>
            {brand?.logo
              ? <img src={brand.logo} alt="Logo" style={{ height: 34, objectFit: 'contain' }} onError={e => { e.target.style.display = 'none'; }} />
              : <div style={{ fontSize: 16, fontWeight: 900, color: isAdmin ? ac : '#1A1410' }}>{brand?.name || 'Glasstech'}</div>
            }
            <div style={{ width: 60 }} />
          </div>
        )}
        ...
        {!isMobile && (
          <div style={{ textAlign: 'center', marginBottom: 36, position: 'relative', zIndex: 1 }}>
            {brand?.logo
              ...
              : <div style={{ fontSize: 17, fontWeight: 900, color: isAdmin ? '#fff' : '#1A1410', marginBottom: 20 }}>{brand?.name || 'Glasstech'}</div>
            }
          </div>
        )}
        ...
        <div style={{ textAlign: 'center', marginBottom: 28, position: 'relative', zIndex: 1 }}>
          {!isAdmin && step === 'phone' && (
            <>
              <div style={{ fontSize: isMobile ? 26 : 28, fontWeight: 900, color: '#1A1410', marginBottom: 10, lineHeight: 1.2 }}>Welcome back</div>
              <div style={{ fontSize: isMobile ? 15 : 14, color: '#7A6E62', lineHeight: 1.6, maxWidth: 300, margin: '0 auto' }}>
          ...
          {!isAdmin && step === 'otp' && (
            <>
              <div style={{ fontSize: isMobile ? 26 : 28, fontWeight: 900, color: '#1A1410', marginBottom: 10, lineHeight: 1.2 }}>Check your SMS</div>
              <div style={{ fontSize: isMobile ? 15 : 14, color: '#7A6E62', lineHeight: 1.6 }}>
                Code sent to&nbsp;<span style={{ fontWeight: 800, color: '#1A1410' }}>{country.flag} {country.code} {phone}</span>
              </div>
            </>
          )}
          {isAdmin && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: 14, border: '1.5px solid #E8E3DD',
                  background: '#F9F7F4', fontSize: 16,
                  outline: 'none', color: '#1A1410', fontFamily: 'inherit',
                  touchAction: 'manipulation',
                }}
              />
            </div>
            <button
              ...
              style={{
                ...
                background: sanitizePhone(phone).length >= 6 ? '#1A1410' : '#E8E3DD',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24, position: 'relative', zIndex: 1 }}>
            ...
            {verifyLoading ? (
              <div style={{ textAlign: 'center', padding: '32px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ fontSize: 15, color: '#7A6E62', fontWeight: 600 }}>Verifying your code…</div>
              </div>
            ) : (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <button onClick={() => { setStep('phone'); setClientErr(''); }}
                    style={{ fontSize: 14, fontWeight: 600, color: '#B5AFA9', background: 'none', border: 'none', cursor: 'pointer', padding: '10px 0', touchAction: 'manipulation', minHeight: 44 }}>
                    ← Change number
                  </button>
                  <button onClick={sendOtp} disabled={resendTimer > 0 || otpLoading}
                    style={{ fontSize: 14, fontWeight: 700, color: resendTimer > 0 ? '#B5AFA9' : ac, background: 'none', border: 'none', cursor: resendTimer > 0 ? 'default' : 'pointer', padding: '10px 0', display: 'flex', alignItems: 'center', gap: 5, touchAction: 'manipulation', minHeight: 44 }}>
                    <RefreshCw size={13} />
        ...
        {isAdmin && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, position: 'relative', zIndex: 1 }}>
            ...
            <button onClick={adminLogin} disabled={adminLoading}
              style={{ height: 56, borderRadius: 16, background: ac, color: '#1A1410', border: 'none', fontSize: 16, fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 4, touchAction: 'manipulation' }}>
        ...
        {/* Footer */}
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: `1px solid ${isAdmin ? 'rgba(255,255,255,.06)' : '#F0EBE5'}`, textAlign: 'center', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#B5AFA9' }}><Shield size={11} /> Encrypted</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#B5AFA9' }}><Globe size={11} /> Global</div>
            {!isAdmin && <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#B5AFA9' }}><Smartphone size={11} /> SMS Code</div>}
          </div>
          {mode === 'client'
            ? <button onClick={() => { setMode('admin'); setClientErr(''); setAdminErr(''); setStep('phone'); }}
                style={{ fontSize: 13, color: '#B5AFA9', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: '10px 0', touchAction: 'manipulation', minHeight: 44 }}>
                Staff / Admin Login →
              </button>
