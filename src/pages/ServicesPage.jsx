...
const ALL_SERVICES = [
  {
    ...
    desc: 'We supply and install all types of glass systems across Ghana — from frameless shower enclosures and glass balustrades to full curtain wall systems for commercial buildings. Our glass is sourced directly from certified factories, giving you tempered, laminated, and frosted options at the best prices in Ghana.',
  ...
  },
  {
    ...
    desc: 'Our certified electricians handle all electrical installation and upgrade works across Ghana. From rewiring a home to installing smart lighting systems, installing distribution boards, and fitting sockets and switches — we do it all to ECG and industry standards. We also supply and install LED lighting, chandeliers, and smart switches.',
...
export default function ServicesPage({ brand, navigate }) {
  ...
  return (
    <div style={{ background: LIGHT_BG }}>
      {/* Hero */}
      <section style={{ padding: mob ? '120px 24px 80px' : '180px 5vw 100px', background: '#1A1410', color: '#fff' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: mob ? 15 : 18, color: 'rgba(255,255,255,0.55)', maxWidth: 600, lineHeight: 1.7, marginBottom: 40 }}>
            Glass, aluminium, bathrooms, kitchens, wardrobes, tiles, doors, electrical, and plumbing — we handle it all under one roof across Ghana.
          </p>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 20, flex: 1 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: '#B5AFA9', margin: 0, fontWeight: 500 }}>{svc.tagline}</p>
                  </div>
                </div>
                <div style={{ width: 36, height: 36, borderRadius: 12, background: isOpen ? ac : '#F9F7F4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.25s' }}>
                  <ChevronRight size={18} color={isOpen ? '#fff' : '#B5AFA9'} style={{ transform: isOpen ? 'rotate(90deg)' : 'none', transition: 'transform 0.25s' }} />
                </div>
              </button>
              ...
              {isOpen && (
                <div style={{ padding: mob ? '0 28px 32px' : '0 48px 48px', borderTop: '1px solid rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: 48, paddingTop: 32 }}>
                    <div>
                      <p style={{ fontSize: 15, color: 'rgba(26,20,16,0.65)', lineHeight: 1.8, marginBottom: 32 }}>{svc.desc}</p>
                    ...
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 800, color: '#B5AFA9', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 16 }}>What we cover</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {svc.items.map(item => (
                          <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', background: '#F9F7F4', borderRadius: 12 }}>
      ...
      {/* Areas we serve */}
      <section style={{ padding: mob ? '60px 24px' : '100px 5vw', background: '#1A1410', color: '#fff' }}>
      ...
      <section style={{ padding: mob ? '60px 24px' : '100px 5vw', background: '#FDFCFB' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 15, color: '#B5AFA9', marginBottom: 40, lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a
              href={`https://wa.me/${whatsappBase}?text=${encodeURIComponent("Hi Glasstech, I'd like to discuss a project and get a free quote.")}`}
              target="_blank" rel="noopener noreferrer"
